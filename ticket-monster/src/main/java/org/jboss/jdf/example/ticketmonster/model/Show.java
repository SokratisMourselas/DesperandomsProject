...
@Entity
@Table(uniqueConstraints = @javax.persistence.UniqueConstraint(columnNames = { "event_id", "venue_id" }))
...
//	 The synthetic id of the object.
	...
	* attribute establishes this. We cascade all persistence operations to the set of  performances, so, for example if a show is removed, then all of it’s performances will also be removed.
    * Normally a collection is loaded from the database in the order of the rows, but here we want to make sure that performances are ordered by date - we let the RDBMS do the heavy lifting. The
